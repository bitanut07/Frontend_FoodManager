...
function EditVoucher() {
  ...
  const fetchVoucher = async () => {
    try {
      ...
      if (foundVoucher) {
        setVoucher({
          ...foundVoucher,
          start_date: foundVoucher.start_date?.split('T')[0] || "",
          end_date: foundVoucher.end_date?.split('T')[0] || "",
        });
        if (foundVoucher.image) {
          setPreviewImage(foundVoucher.image);
        }
      } else {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      await updateVoucher(id, voucher);
      setModal({
        isOpen: true,
